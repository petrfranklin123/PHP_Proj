...
/*
    //API это интерфейс, с которым приходится работать 
    //Мы будем работать с фейковым сервером json https://jsonplaceholder.typicode.com/
    //для начала мы отправим GET запрос через запрос fetch()
    //fetch возвращает паромис, с которым можно последовательно работать 
    //в данном коде мы получаем первую запись 
    fetch('https://jsonplaceholder.typicode.com/todos/1')
    //переделываем ее в обычный объект JS методом response
    //response.json() – декодирует ответ в формате JSON,
    //response.text() – читает ответ и возвращает как обычный текст,
        .then(response => response.json())
        //последоватиельно выводим в консоль объект
        .then(json => console.log(json));
    //второй способ вышестоящего запроса 
    fetch('https://jsonplaceholder.typicode.com/todos/1')
        .then((response) => response.json())
        .then((json) => {
            console.log(json);
    ...
    //организация POST запросов (Записей)
    fetch('https://jsonplaceholder.typicode.com/posts', 
    //создаем объект с настройками 
    {
        //указываем название запроса 
        method: "POST",
        //в тело записываем объект и кодируем его сразу в JSON 
        body: JSON.stringify({
            name: 'Alex',
            nameID: '123'
        }),
        //указываем заголовочные данные, которые определяют, какого плана объект мы отправляем 
        headers: {
            'Content-type': 'application/json'
        }
        //при осуществлении запроса, мы получим ответ в промис 
    }).then(response => response.json())
    //последоватиельно выводим в консоль объект
    .then(json => console.log(json));
    */
